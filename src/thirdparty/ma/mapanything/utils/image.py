...
def _get_image_normalization_dict():
...
def rgb(ftensor, norm_type, true_shape=None):
    ...
    if ftensor.dtype == np.uint8:
        ...
        if norm_type in image_normalization_dict.keys():
            ...
            raise ValueError(
                "Unknown image normalization type: "
                f"{norm_type}. Available types: identity or "
                f"{image_normalization_dict.keys()}"
            )
...
def load_images(
    ...
    if norm_type in image_normalization_dict.keys():
        ...
        raise ValueError(
            "Unknown image normalization type: "
            f"{norm_type}. Available options: "
            f"{list(image_normalization_dict.keys())}"
        )

...
from ......interface.geometric_model import MultiViewInput, PoseConvention, SceneGeometry, TensorProbe
from ...utils.device import empty_cache, get_amp_dtype, get_autocast_device_type
...
from ...utils.image import _get_image_normalization_dict
...
try:
    ...
    _UNICEPTION_IMPORT_ERROR: ModuleNotFoundError | None = None
except ModuleNotFoundError as exc:  # pragma: no cover - depends on optional runtime dependency
    EncoderGlobalRepInput = Any
    ViTEncoderInput = Any
    ViTEncoderNonImageInput = Any
    encoder_factory = None
    MultiViewAlternatingAttentionTransformer = Any
    MultiViewAlternatingAttentionTransformerIFR = Any
    MultiViewTransformerInput = Any
    MultiViewCrossAttentionTransformer = Any
    MultiViewCrossAttentionTransformerIFR = Any
    MultiViewGlobalAttentionTransformer = Any
    MultiViewGlobalAttentionTransformerIFR = Any
    CamTranslationPlusQuatsAdaptor = Any
    PointMapAdaptor = Any
    PointMapPlusRayDirectionsPlusDepthAdaptor = Any
    PointMapPlusRayDirectionsPlusDepthWithConfidenceAdaptor = Any
    PointMapPlusRayDirectionsPlusDepthWithConfidenceAndMaskAdaptor = Any
    PointMapPlusRayDirectionsPlusDepthWithMaskAdaptor = Any
    PointMapWithConfidenceAdaptor = Any
    PointMapWithConfidenceAndMaskAdaptor = Any
    PointMapWithMaskAdaptor = Any
    RayDirectionsPlusDepthAdaptor = Any
    RayDirectionsPlusDepthWithConfidenceAdaptor = Any
    RayDirectionsPlusDepthWithConfidenceAndMaskAdaptor = Any
    RayDirectionsPlusDepthWithMaskAdaptor = Any
    RayMapPlusDepthAdaptor = Any
    RayMapPlusDepthWithConfidenceAdaptor = Any
    RayMapPlusDepthWithConfidenceAndMaskAdaptor = Any
    RayMapPlusDepthWithMaskAdaptor = Any
    ScaleAdaptor = Any
    AdaptorInput = Any
    PredictionHeadInput = Any
    PredictionHeadLayeredInput = Any
    PredictionHeadTokenInput = Any
    DPTFeature = Any
    DPTRegressionProcessor = Any
    LinearFeature = Any
    MLPHead = Any
    PoseHead = Any
    Mlp = Any
    SwiGLUFFNFused = Any
    _UNICEPTION_IMPORT_ERROR = exc
...
def _require_uniception() -> None:
    if _UNICEPTION_IMPORT_ERROR is None:
        return
    raise ModuleNotFoundError(
        "MapAnything requires the optional 'uniception' package to instantiate or run inference."
    ) from _UNICEPTION_IMPORT_ERROR
...
class MapAnything(nn.Module, PyTorchModelHubMixin):
    ...
    def patch_start_index(self) -> int:
        return 1 + int(self.encoder.model.num_register_tokens) # type: ignore

    def inject_probe(self, block_index: int, probe: TensorProbe) -> None:
        vit: dinov2.DINOv2Encoder = self.encoder.model # type: ignore
        blocks = cast(nn.ModuleList, vit.blocks)
        patch_start = self.patch_start_index

        def hook_fn(module: nn.Module, args: tuple[Any, ...], output: torch.Tensor | tuple[torch.Tensor, ...]) -> None:
            if isinstance(output, tuple):
                if len(output) == 0 or not isinstance(output[0], torch.Tensor):
                    raise TypeError("Expected encoder block hook output to contain a tensor in position 0.")
                token_tensor = output[0]
            elif isinstance(output, torch.Tensor):
                token_tensor = output
            else:
                raise TypeError(f"Unsupported encoder block hook output type: {type(output)!r}")
            probe.data = token_tensor[:, patch_start:]

        blocks[block_index].register_forward_hook(hook_fn)

    def _get_encoder_vit(self) -> Any:
        pretrained = getattr(self.encoder, "pretrained", None)
        if pretrained is None:
            raise RuntimeError("MapAnything probe support requires self.encoder.pretrained to exist.")
        if not hasattr(pretrained, "blocks"):
            raise RuntimeError("MapAnything probe support requires self.encoder.pretrained.blocks to exist.")
        if not hasattr(pretrained, "num_register_tokens"):
            raise RuntimeError(
                "MapAnything probe support requires self.encoder.pretrained.num_register_tokens to exist."
            )
        return pretrained

    def _get_data_norm_type(self) -> str:
        data_norm_type = getattr(self.encoder, "data_norm_type", None)
        if data_norm_type is None:
            data_norm_type = self.encoder_config.get("data_norm_type")
        if not isinstance(data_norm_type, str):
            raise RuntimeError("MapAnything could not determine the encoder data normalization type.")
        return data_norm_type

    @staticmethod
    def _norm_stat_to_image_tensor(stat: Any, images: torch.Tensor) -> torch.Tensor:
        """Place encoder mean/std on ``images`` device/dtype without ``new_tensor(other_tensor)`` (PyTorch warns on that)."""
        if isinstance(stat, torch.Tensor):
            return stat.to(device=images.device, dtype=images.dtype)
        return torch.as_tensor(stat, device=images.device, dtype=images.dtype)
    ...
    def _normalize_multiview_input(self, input: MultiViewInput) -> torch.Tensor:
        ...
        patch_size = int(getattr(self.encoder, "patch_size", 1))
        if patch_size > 1 and (images.shape[-2] % patch_size != 0 or images.shape[-1] % patch_size != 0):
            raise ValueError(
                f"Expected image height/width divisible by {patch_size}, got {tuple(images.shape[-2:])}"
            )
        ...
        if image_min < 0.0 or image_max > 1.0:
            raise ValueError(
                f"Expected unnormalized image values in [0, 1] before MapAnything normalization, got range [{image_min}, {image_max}]"
            )
        ...
        data_norm_type = self._get_data_norm_type()
        if data_norm_type == "identity":
            return images

        image_normalization_dict = _get_image_normalization_dict()
        if data_norm_type not in image_normalization_dict:
            raise ValueError(
                f"Unknown MapAnything encoder normalization type: {data_norm_type!r}. "
                f"Available types: {sorted(image_normalization_dict.keys())}"
            )

        img_norm = image_normalization_dict[data_norm_type]
        mean = self._norm_stat_to_image_tensor(img_norm.mean, images).view(1, 1, 3, 1, 1)
        std = self._norm_stat_to_image_tensor(img_norm.std, images).view(1, 1, 3, 1, 1)
        return (images - mean) / std

    def _multiview_input_to_views(
        self,
        input: MultiViewInput,
        normalized_images: torch.Tensor,
    ) -> list[dict[str, Any]]:
        data_norm_type = self._get_data_norm_type()
        _, num_views = normalized_images.shape[:2]
        views: list[dict[str, Any]] = []
        for view_index in range(num_views):
            view: dict[str, Any] = {
                "img": normalized_images[:, view_index].contiguous(),
                "data_norm_type": [data_norm_type],
            }
            if input.intrinsics is not None:
                view["intrinsics"] = input.intrinsics[:, view_index].contiguous()
            views.append(view)
        return views

    @staticmethod
    def _stack_optional_view_tensor(
        infer_outputs: list[dict[str, torch.Tensor]],
        key: str,
        transform: Callable[[torch.Tensor], torch.Tensor],
    ) -> torch.Tensor | None:
        presence = [key in output for output in infer_outputs]
        if not any(presence):
            return None
        if not all(presence):
            raise RuntimeError(f"Inconsistent MapAnything infer outputs for key {key!r}.")
        return torch.stack([transform(output[key]) for output in infer_outputs], dim=1)

    @staticmethod
    def _camera_poses_to_xyzw(camera_poses: torch.Tensor) -> torch.Tensor:
        rotation = camera_poses[:, :3, :3]
        translation = camera_poses[:, :3, 3]
        if camera_poses.device.type == "cuda":
            with torch.autocast(device_type="cuda", enabled=False):
                quaternion = pp.from_matrix(rotation, ltype=pp.SO3_type, check=False).tensor()
        else:
            quaternion = pp.from_matrix(rotation, ltype=pp.SO3_type, check=False).tensor()
        return torch.cat([translation, quaternion], dim=-1)

    def _infer_outputs_to_scene_geometry(
        self,
        infer_outputs: list[dict[str, torch.Tensor]],
    ) -> SceneGeometry:
        depths = self._stack_optional_view_tensor(
            infer_outputs,
            "depth_z",
            lambda value: value.permute(0, 3, 1, 2).contiguous(),
        )
        points = self._stack_optional_view_tensor(
            infer_outputs,
            "pts3d",
            lambda value: value.permute(0, 3, 1, 2).contiguous(),
        )
        intrinsics = self._stack_optional_view_tensor(infer_outputs, "intrinsics", lambda value: value.contiguous())
        confidence = self._stack_optional_view_tensor(
            infer_outputs,
            "conf",
            lambda value: value.unsqueeze(1).contiguous(),
        )
        infer_mask = self._stack_optional_view_tensor(
            infer_outputs,
            "mask",
            lambda value: (~value.permute(0, 3, 1, 2).bool()).contiguous(),
        )
        poses = self._stack_optional_view_tensor(
            infer_outputs,
            "camera_poses",
            self._camera_poses_to_xyzw,
        )

        return SceneGeometry(
            pose_convention=(PoseConvention.R, PoseConvention.D, PoseConvention.F),
            depths=depths,
            depths_conf=confidence,
            points=points,
            points_conf=confidence,
            poses=poses,
            intrinsics=intrinsics,
            infer_mask=infer_mask,
        )

    def _raw_outputs_to_scene_geometry(
        self,
        raw_outputs: List[Dict[str, torch.Tensor]],
    ) -> SceneGeometry:
        """Build SceneGeometry directly from ``_forward_processed_views`` output.

        Bypasses ``postprocess_model_outputs_for_inference`` entirely, avoiding:
        - ``recover_pinhole_intrinsics_from_ray_directions`` (dominant GPU cost)
        - CPU-side numpy edge masking
        - cam_trans/cam_quats -> 4x4 matrix -> [t | q_xyzw] round-trip
        - Image denormalization
        """
        points = self._stack_optional_view_tensor(
            raw_outputs, "pts3d",
            lambda v: v.permute(0, 3, 1, 2).contiguous(),
        )

        depths = self._stack_optional_view_tensor(
            raw_outputs, "pts3d_cam",
            lambda v: v[..., 2:3].permute(0, 3, 1, 2).contiguous(),
        )

        confidence = self._stack_optional_view_tensor(
            raw_outputs, "conf",
            lambda v: v.unsqueeze(1).contiguous(),
        )

        has_trans = ["cam_trans" in out for out in raw_outputs]
        has_quats = ["cam_quats" in out for out in raw_outputs]
        if any(has_trans) != all(has_trans) or any(has_quats) != all(has_quats):
            raise RuntimeError(
                "Inconsistent raw outputs: cam_trans/cam_quats must be present in all or no views."
            )
        poses: torch.Tensor | None = None
        if all(has_trans) and all(has_quats):
            poses = torch.stack(
                [torch.cat([out["cam_trans"], out["cam_quats"]], dim=-1) for out in raw_outputs],
                dim=1,
            )

        has_ray_dirs = ["ray_directions" in out for out in raw_outputs]
        if any(has_ray_dirs) != all(has_ray_dirs):
            raise RuntimeError(
                "Inconsistent raw outputs: ray_directions must be present in all or no views."
            )
        intrinsics: torch.Tensor | None = None
        if all(has_ray_dirs):
            intrinsics = torch.stack(
                [recover_pinhole_intrinsics_from_ray_directions(out["ray_directions"]) for out in raw_outputs],
                dim=1,
            )
    ...
    def _forward_multiview_input(
        self,
        input: MultiViewInput,
        memory_efficient_inference: bool,
        minibatch_size: int | None,
    ) -> SceneGeometry:
        normalized_images = self._normalize_multiview_input(input)
        views = self._multiview_input_to_views(input, normalized_images)

        self._configure_geometric_input_config(
            use_calibration=False, use_depth=False, use_pose=False,
            use_depth_scale=False, use_pose_scale=False,
        )

        raw_outputs = self._forward_processed_views(
            views,
        ...
        return self._raw_outputs_to_scene_geometry(raw_outputs)
    ...
    def _forward_processed_views(
        self,
        views: List[Dict[str, Any]],
        memory_efficient_inference: bool,
        minibatch_size: int | None,
    ) -> List[Dict[str, torch.Tensor]]:
        """Run the raw MapAnything prediction stack on preprocessed per-view inputs."""
        ...
        with nvtx.range("ma_encode_n_views"):
        ...
        with nvtx.range("ma_info_sharing"):
            if self.info_sharing_return_type == "no_intermediate_features":
        ...
        with torch.autocast(get_autocast_device_type(self.device), enabled=False):
            ...
            with nvtx.range("ma_downstream_head"):
    ...
    def forward(
        self,
        views: MultiViewInput | List[Dict[str, Any]],
        memory_efficient_inference: bool = True,
        minibatch_size: int | None = None,
    ) -> SceneGeometry | List[Dict[str, torch.Tensor]]:
        """
        Compatibility wrapper that routes list-based calls through ``infer``.

        ``MultiViewInput`` still returns ``SceneGeometry`` to match the current
        integration contract.
        """
        if isinstance(views, MultiViewInput):
            return self._forward_multiview_input(
                views,
                memory_efficient_inference=memory_efficient_inference,
                minibatch_size=minibatch_size,
            )
    ...
    def infer(
        ...
        with torch.autocast(device_type, enabled=bool(use_amp), dtype=amp_dtype):
            preds = self._forward_processed_views(
                processed_views,
                memory_efficient_inference=memory_efficient_inference,
                minibatch_size=minibatch_size,
            )

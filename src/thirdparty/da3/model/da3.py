...
from ....interface.geometric_model import TensorProbe
...
class DepthAnything3Net(nn.Module):
    ...
    def patch_start_index(self) -> int:
        vit: Any = self.backbone.pretrained
        return 1 + int(vit.num_register_tokens)

    def inject_probe(self, block_index: int, probe: TensorProbe) -> None:
        vit: Any = self.backbone.pretrained
        blocks = cast(torch.nn.ModuleList, vit.blocks)
        patch_start = self.patch_start_index

        def hook_fn(module: nn.Module, args: tuple, output: torch.Tensor) -> None:
            probe.data = output[:, patch_start:]
    ...
    def forward(
        ...
        """
        Raw forward pass through the network (tensor-in, dict-out).
...
class NestedDepthAnything3Net(nn.Module):
    ...
    @property
    def patch_start_index(self) -> int:
        return self.da3.patch_start_index

    def inject_probe(self, block_index: int, probe: TensorProbe) -> None:
        self.da3.inject_probe(block_index, probe)
    ...
    def forward(
        ...
        """
        Raw forward pass through both branches with metric scaling alignment.
        ...
        metric_output = self.da3_metric._forward_raw(x)

...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        if ffn_layer == "mlp":
            Diagnostics.log("using MLP layer as FFN")
            ffn_layer = Mlp
        elif ffn_layer == "swiglufused" or ffn_layer == "swiglu":
            Diagnostics.log("using SwiGLU layer as FFN")
            ffn_layer = SwiGLUFFNFused
        elif ffn_layer == "identity":
            Diagnostics.log("using Identity layer as FFN")
    ...
    def _get_intermediate_layers_not_chunked(self, x, n=1, export_feat_layers=[], **kwargs):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if self.alt_start != -1 and (i == self.alt_start - 1) and x.shape[1] >= THRESH_FOR_REF_SELECTION and kwargs.get("cam_token", None) is None:
                ...
                Diagnostics.log(f"Selecting reference view using strategy: {strategy}")
            ...
            if self.alt_start != -1 and i == self.alt_start:
                if kwargs.get("cam_token", None) is not None:
                    Diagnostics.log("Using camera conditions provided by the user")

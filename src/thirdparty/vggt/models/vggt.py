...
class VGGT(nn.Module, PyTorchModelHubMixin):
    def __init__(self, img_size: int | tuple[int, int] = 518, patch_size=14, embed_dim=1024):
        super().__init__()
        ...
        retain_layers = (
            frozenset(self.point_head.intermediate_layer_idx)
            | frozenset(self.depth_head.intermediate_layer_idx)
        )
    ...
    def forward(self, input: "MultiViewInput") -> "SceneGeometry":
        """
        Forward pass compatible with ``interface.geometric_model.GeometricPredictorLike``.

        Args:
            input: ``MultiViewInput`` with:
                - images: [B, S, 3, H, W]
                - intrinsics: [B, S, 3, 3] or None

        Returns:
            SceneGeometry with:
                - depths: [B, S, 1, H, W]
                - depths_conf: [B, S, 1, H, W] or None
                - points: [B, S, 3, H, W] or None
                - points_conf: [B, S, 1, H, W] or None
                - poses: [B, S, 7]
                - intrinsics: [B, S, 3, 3]
        """
        images = input.images

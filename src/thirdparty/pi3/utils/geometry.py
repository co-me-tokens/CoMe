...
def intrinsics_from_xy(xy: torch.Tensor, H: int, W: int) -> torch.Tensor:
    """Estimate pinhole intrinsics from predicted per-pixel ray directions.

    Pi3/Pi3X predict xy = [X/Z, Y/Z] per pixel using the pixel-center convention
    (pixel (w, h) has center at (w+0.5, h+0.5)).  The pinhole relationship is:

        xy[..., h, w, 0] = (w + 0.5 - cx) / fx
        xy[..., h, w, 1] = (h + 0.5 - cy) / fy

    with cx = W/2 and cy = H/2 assumed.  fx and fy are recovered by taking the
    mean of (displacement / predicted_xy) over pixels whose displacement magnitude
    exceeds a small threshold.

    Args:
        xy: Float tensor [B, N, H, W, 2] of predicted (X/Z, Y/Z) values.
        H:  Image height in pixels.
        W:  Image width in pixels.

    Returns:
        K:  Float tensor [B, N, 3, 3] — standard pinhole intrinsics matrix.

    Raises:
        ValueError: If xy does not have exactly 5 dimensions.
    """
    if xy.ndim != 5:
        raise ValueError(f"xy must be [B, N, H, W, 2], got shape {tuple(xy.shape)}")
    ...
    cx = W * 0.5
    cy = H * 0.5
    _EPS = 1.0  # minimum pixel displacement to include in the average

    ws = torch.arange(W, device=device, dtype=dtype) + 0.5  # [W]
    hs = torch.arange(H, device=device, dtype=dtype) + 0.5  # [H]

    dx = (ws - cx).view(1, 1, 1, W)   # [1,1,1,W]
    dy = (hs - cy).view(1, 1, H, 1)   # [1,1,H,1]

    mask_x = dx.abs() >= _EPS  # [1,1,1,W]
    mask_y = dy.abs() >= _EPS  # [1,1,H,1]

    # For each (b, n): fx = mean over valid pixels of dx / xy[...,0]
    xy_x = xy[..., 0]  # [B, N, H, W]
    xy_y = xy[..., 1]  # [B, N, H, W]

    valid_x = mask_x.expand(B, N, H, W) & (xy_x.abs() >= 1e-6)
    valid_y = mask_y.expand(B, N, H, W) & (xy_y.abs() >= 1e-6)

    safe_xy_x = torch.where(valid_x, xy_x, torch.ones_like(xy_x))
    safe_xy_y = torch.where(valid_y, xy_y, torch.ones_like(xy_y))
    fx_vals = dx.expand(B, N, H, W) / safe_xy_x
    fy_vals = dy.expand(B, N, H, W) / safe_xy_y

    # Masked mean per (B, N)
    fx = (fx_vals * valid_x).sum(dim=(-2, -1)) / valid_x.sum(dim=(-2, -1)).clamp(min=1)
    fy = (fy_vals * valid_y).sum(dim=(-2, -1)) / valid_y.sum(dim=(-2, -1)).clamp(min=1)

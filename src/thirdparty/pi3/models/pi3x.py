...
from ..utils.geometry import se3_inverse, get_pixel, homogenize_points, intrinsics_from_xy
...
class Pi3X(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(self, input: MultiViewInput) -> SceneGeometry:
        ...
        if input.intrinsics is not None:
            out_intrinsics = input.intrinsics
        else:
            # Recover xy = [X/Z, Y/Z] from local_points = [xy*z, z]; shape [B,N,H,W,2]
            lp = outputs['local_points']  # [B, N, H, W, 3]
            xy_pred = lp[..., :2] / lp[..., 2:3].clamp(min=1e-6)
            out_intrinsics = intrinsics_from_xy(xy_pred.detach(), H, W)

        return SceneGeometry(
            pose_convention=(PoseConvention.R, PoseConvention.D, PoseConvention.F),
            depths=outputs['local_points'][..., 2:3].permute(0, 1, 4, 2, 3).contiguous(),
            depths_conf=outputs['conf'].permute(0, 1, 4, 2, 3).contiguous(),
            points=outputs['points'].permute(0, 1, 4, 2, 3).contiguous(),
            points_conf=outputs['conf'].permute(0, 1, 4, 2, 3).contiguous(),
            poses=poses,

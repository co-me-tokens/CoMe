...
class GlobalAttentionConfidence(torch.nn.Module):
    def __init__(self, hidden_dims: T.Sequence[int], activation: str, num_register: int=0):
        super().__init__()

        self.proj1      = torch.nn.Linear(in_features=hidden_dims[0], out_features=hidden_dims[1])
        self.block      = Block(dim=hidden_dims[1], num_heads=1, mlp_ratio=2)
        ...
        if num_register > 0:
            self.register_token = torch.nn.Parameter(torch.zeros(1, num_register, hidden_dims[1]))
        else:
    ...
    def forward(self, tokens: torch.Tensor, BPHWC: tuple[int, ...]) -> torch.Tensor:
        ...
        if self.register_token is not None:
            ...
            tokens = torch.cat([register_token, tokens], dim=1)

        with nvtx.range(f"conf_global_attn_block(seqlen={tokens.shape[1]})"):
            tokens = self.block(tokens)

        if self.register_token is not None:
            tokens = tokens[:, self.num_register:, :]

        with nvtx.range("conf_proj2"):

...
from ....thirdparty.pi3.models.layers.block     import Block
...
class JaggedBlock(torch.nn.Module):
    def __init__(self, original_block: Block, attn_dtype: JaggedAttention.SupportType):
        ...
        self.attn  = JaggedAttention(cast(AttentionRope, original_block.attn), run_dtype=attn_dtype)
...
class JaggedBlock_w_Bias(torch.nn.Module):
    def __init__(self, original_block: Block, attn_dtype: JaggedAttention.SupportType):
        super().__init__()
        self.block = original_block
        self.attn  = JaggedAttention_w_Bias(cast(AttentionRope, original_block.attn), run_dtype=attn_dtype)

...
void set_params_perkey_bias(Flash_fwd_params &params, std::optional<at::Tensor> &perkey_bias_, int total_k, at::ScalarType expected_dtype) {
    if (perkey_bias_.has_value()) {
        auto perkey_bias = perkey_bias_.value();
        TORCH_CHECK(perkey_bias.dtype() == expected_dtype,
                     "perkey_bias dtype must match QKV dtype (", expected_dtype,
                     "), got ", perkey_bias.dtype());
        CHECK_DEVICE(perkey_bias);
        CHECK_CONTIGUOUS(perkey_bias);
        TORCH_CHECK(perkey_bias.sizes() == torch::IntArrayRef({total_k}),
                     "perkey_bias must have shape (total_k,), got ", perkey_bias.sizes());
        params.perkey_bias_ptr = perkey_bias.data_ptr();
    } else {
        params.perkey_bias_ptr = nullptr;
    }
}


std::vector<at::Tensor>
mha_varlen_fwd_impl(at::Tensor &q,
               const at::Tensor &k,
               const at::Tensor &v,
    ...
    const int total_k = paged_KV ? 0 : k.size(0);
    set_params_perkey_bias(params, perkey_bias_, total_k, q.scalar_type());
...
std::vector<at::Tensor>
mha_varlen_fwd_w_perkey_bias(at::Tensor &q,
               const at::Tensor &k,
               const at::Tensor &v,
               std::optional<at::Tensor> &out_,
               const at::Tensor &cu_seqlens_q,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.doc() = "FlashAttention -- forward-only varlen build";
    m.def("varlen_fwd", &FLASH_NAMESPACE::mha_varlen_fwd, "Forward pass (variable length)");
    m.def("varlen_fwd_w_perkey_bias", &FLASH_NAMESPACE::mha_varlen_fwd_w_perkey_bias, "Forward pass (variable length) with per-key bias");
}

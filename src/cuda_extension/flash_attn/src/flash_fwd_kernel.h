...
template<int kBlockN, typename BiasElem, typename Engine, typename Layout>
__forceinline__ __device__ void apply_perkey_bias_block(
    Tensor<Engine, Layout> &acc_s,
    const BiasElem *bias_base,
    const int n_block,
    const int actual_seqlen_k,
    const float inv_scale_softmax) {
    Tensor scores = make_tensor(acc_s.data(), FLASH_NAMESPACE::convert_layout_acc_rowcol(acc_s.layout()));
    const int lane_id = threadIdx.x % 32;
    const int col_idx_offset = n_block * kBlockN + (lane_id % 4) * 2;
    #pragma unroll
    for (int nj = 0; nj < size<1, 1>(scores); ++nj) {
        ...
        for (int j = 0; j < size<1, 0>(scores); ++j) {
            ...
            if (col_idx < actual_seqlen_k) {
                const float bias_val = static_cast<float>(bias_base[col_idx]) * inv_scale_softmax;
                #pragma unroll
                for (int mi = 0; mi < size<0>(scores); ++mi) {
                    scores(mi, make_coord(j, nj)) += bias_val;
                }
            }
        }
    }
...
inline __device__ void compute_attn_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    const Element *perkey_bias_base = nullptr;
    float perkey_bias_inv_scale = 0.0f;
    if (params.perkey_bias_ptr != nullptr) {
        const int bias_offset = binfo.sum_s_k >= 0 ? binfo.sum_s_k : bidb * params.seqlen_k;
        perkey_bias_base = reinterpret_cast<const Element *>(params.perkey_bias_ptr) + bias_offset;
        perkey_bias_inv_scale = 1.0f / params.scale_softmax;
    }
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (perkey_bias_base != nullptr) {
            FLASH_NAMESPACE::apply_perkey_bias_block<kBlockN, Element>(
                acc_s, perkey_bias_base,
                n_block, binfo.actual_seqlen_k,
                perkey_bias_inv_scale);
        }
